...
L'explicabilit\'e est au cœur de la proposition de valeur de XAI Studio.
...
\begin{itemize}
    ...
    \item \textbf{Beeswarm plot}\,---\,chaque point repr\'esente une
        instance ; la position horizontale encodes la contribution
...
L'implémentation détecte automatiquement si le mod\`ele est
...
d\'egradation est grande, plus la variable est \og importante \fg{}.

Cette approche est \textbf{model-agnostic}\,---\,elle fonctionne sur
tout mod\`ele sklearn\,---\,et offre une perspectivité compl\'ementaire
\`a SHAP : tant\^ot qu'une feature a \textit{une grosse variance d'effet},
elle peut apparaitre importante en permutation mais pas SHAP.

\section{Explications locales\,---\,LIME et SHAP par instance}

\subsection{LIME\,---\,approximation lin\'eaire locale}

Pour expliquer \textit{une seule} pr\'ediction, LIME procède ainsi~:

\begin{enumerate}
    \item Gen\'ere des instances \textit{perturbées} autour du point
        d'intérêt en modifiant ses features de mani\`ere al\'eatoire.
...
intuitiv\'ité. Son inconv\'enient~: d\'epend d'un tirage al\'eatoire,
...
SHAP propose aussi deux rendu locaux avanc\'es~:
...
\begin{description}[font=\sffamily\bfseries\color{accent}]
    \item[Waterfall plot]
        ...
        d'une feature. Facilite la compr\'ehension du \og chemin \fg{}
        de la pr\'ediction.
...
Le graphique résultant affiche comment la pr\'ediction moyenne varie
quand $x_j$ change. Utile pour d\'etectrer des relations non-lin\'eaires
...
encodes la pr\'ediction moyenne. Les concentrations de couleur
r\'ev\`elent des effets d'interaction forts.

\section{Caching et optimisation}

Le \texttt{XAIService} maintient deux caches dict internes~:
...
\begin{description}[font=\sffamily\bfseries\color{accent}]
    \item[SHAP]
        ...
        \textit{waterfall}, \textit{force}). \textit{Spinbox} pour
    ...
    \item[Feature Importance]
        Bouton Run unique. Graphique en barres trié.
\end{description}

Toutes les opérations (SHAP, LIME, PDP) sont $\textbf{asynchrones}$~:
...
(femmes, minorit\'es, âges extr\^emes). XAI Studio int\`egre un module
\textit{fairness} complet pour d\'etectrer et quantifier ces biais.
...
\begin{enumerate}
    \item \textbf{Acc\'es par groupe}\,---\,calcul de l'accuracy
        ...
        attribut sensible (sexe, âge, ethnie).
    ...
    \item \textbf{Disparate Impact Ratio}\,---\,ratio des taux de
        pr\'edictions positives. Traditionnellement~: 0.8 (80\% rule)
        est consid\'er\'e comme acceptable en droit du travail.
\end{enumerate}

\subsection{Métriques par groupe}
...
\section{Détection d'alertes}

La fonction \texttt{compute\_fairness\_metrics()} retourne un
dictionnaire incluant un champ \texttt{alerts} (liste d'cha\^ines
descriptives). Les seuils hard-cod\'es sont~:
...
\midrule
Disparate Impact & $< 0.8$ ou $> 1.25$ & \og Impact disproportionn\'e d\'etect\'e \fg{} \\
Demographic Parity & $> 0.1$ & \og Disparit\'e d\'emographique significative \fg{} \\
Equalized Odds (TPR) & $> 0.1$ & \og TPR in\'egal entre groupes \fg{} \\
Equalized Odds (FPR) & $> 0.1$ & \og FPR in\'egal entre groupes \fg{} \\
\bottomrule
\end{tabular}
\end{center}

Chaque alerte est rendue dans la vue avec un badge de couleur
(rouge = critique, orange = avertissement) pour signal une
...
\begin{description}[font=\sffamily\bfseries\color{accent}]
    \item[Sélection d'attribut sensible]
        Liste d\'eroulante peuplée dynamiquement par les colonnes du
        dataset. L'utilisateur choisit l'attribut d\'emographique
        \`a analyser (ex. \og genre \fg{}, \og groupe d'âge \fg{}).
    ...
    \item[Affichage des alertes]
        Banneau (\textit{Alert Box}) listant les biais d\'etect\'es
        avec codes couleur.
    ...
    \item[Graphique comparatif]
        Barre ou courbe compar\'ee entre groupes pour la m\'etrique
...
\textit{threaded} pour ne pas bloquer l'interface. Le r\'esultat
met \`a jour le tableau, les alertes et le graphique via
\texttt{root.after()}.

\section{Intégration au pipeline et bonnes pratiques}
...
\begin{enumerate}
    \item \textbf{Post-entraînement}\,---\,une fois les modèles entraînés
        et la performance globale validée sur l'ensemble de test.
    \item \textbf{Post-prédiction}\,---\,lors d'une inférence batch sur
        de nouvelles données, si les groupes démographiques sont
        disponibles.
\end{enumerate}
...
de biais automatiques (ré-pondération d'exemples, post-processing
de prédictions). Cependant, l'API est conçue pour\,---\,une étape
future pourrait ajouter~:

\begin{itemize}
    \item \textit{Reweighting}\,---\,augmenter les exemples du groupe
        sous-représenté lors du réentraînement.
    \item \textit{Threshold optimization}\,---\,choisir des seuils de
        classification distincts par groupe pour égaliser le taux
        d'acceptation.
    \item \textit{Post-processing}\,---\,modifier les prédictions finales
        pour assurer une parité démographique.
\end{itemize}
...
Au-delà des explications, XAI Studio met l'accent sur la
\textit{visualisation intégrée} des performances et des artefacts
ML via matplotlib embarqué dans Tkinter et, secondairement, Plotly
...
encapsule la génération de graphiques pour~:
...
\begin{description}[font=\sffamily\bfseries\color{accent}]
    \item[Classification]
        ...
        graphiques de gain.
    \item[Régression]
        Graphiques de résidus, graphiques réels vs prédis, histogrammes
        d'erreurs.
    \item[XAI]
        Résumés SHAP (bar, beeswarm), waterfall, force, LIME locaux.
    \item[Fairness]
        Graphiques comparatifs de métriques d'équité par groupe.
\end{description}

Chaque fonction retourne un objet \texttt{PlotPayload} contenant~:
\begin{itemize}
    \item \texttt{figure}\,---\,objet matplotlib \texttt{plt.Figure}.
    \item \texttt{html}\,---\,repreésentation HTML interactive (Plotly)
        si disponible.
    \item \texttt{engine}\,---\,string \og matplotlib \fg{} ou \og plotly \fg{}.
...
Visualise la distribution des vraies/fausses positifs/négatifs.
Rendue sous forme de heatmap avec valeurs numériques incrustées.
La diagonale (prédictions correctes) idéalement dominant pour un bon
classifieur.
...
Trace le TPR (sensibilité) en fonction du FPR (1-spécificité) en
faisant varier le seuil de décision du classificateur. L'aire sous
la courbe (AUC) mesure la qualité globale~: AUC=0.5 (classifieur
aléatoire), AUC=1 (classifieur parfait).

XAI Studio affiche la courbe diagnostique et marque la diagonale
(classifieur aléatoire) en pointillés pour référence.

\section{Plots de régression}

\subsection{Résidus vs prédictions}

Graphique 2D où chaque point représente une observation~:
abscisse = prédiction, ordonnée = résidu (valeur réelle - prédite).
Un nuage homogène proche de 0 indique un bon ajustement. Des
patterns (ex. en V) révèlent une sous-modélisation.

\subsection{Réel vs prédit}

Graphique de dispersion où chaque point est $(y_i^{\text{true}}, y_i^{\text{pred}})$.
Idéalement, tous les points se concentrent sur la diagonale $y=x$.
Les écarts reflètent l'erreur de prédiction.

\section{PlotCanvas\,---\,intégration Tkinter}
...
\begin{description}[font=\sffamily\bfseries\color{accent}]
    \item[Rendu natif]
        Affiche un graphique matplotlib dans une hiérarchie Tkinter.
    \item[Interactions]
        Pan, zoom, sauvegarde PNG via la barre d'outils matplotlib
        native.
    \item[Thèming]
        Applique automatiquement les couleurs du thème courant
        (fond, bordures, axes) via \texttt{apply\_plot\_style()}.
    \item[Redraw]
        Méthode \texttt{redraw(fig)} pour remplacer le graphique affiché
        sans recréer le widget.
\end{description}

\subsection{Export PNG}

Chaque PlotCanvas expose une méthode \texttt{export\_png()}.
Invoque un \texttt{filedialog.asksaveasfilename()} pour choisir le
chemin de sauvegarde, puis écrit le graphique via
\texttt{fig.savefig()}.

\section{Workflow typique d'\'evaluation}

\begin{enumerate}
    \item \textbf{Entraînement}\,---\,l'utilisateur choisit ses modèles et lance
        l'entraînement.
    \item \textbf{Évaluation}\,---\,vue tableau avec performances (accuracy, F1,
        RMSE, R², etc.). Best model en surbrillance.
    \item \textbf{Visualisations}\,---\,clic sur un modèle ouvre un panneau
        latéral affichant confusion matrix, courbe ROC, etc.
    \item \textbf{XAI}\,---\,l'utilisateur navigue vers l'onglet XAI, choisit
        le modèle et la visualisation (SHAP, LIME, PDP).
    \item \textbf{Fairness}\,---\,si données d'attribut sensible dispo,
        analyse des biais par groupe.
\end{enumerate}

La progression est \textit{cohérente} : chaque étape enrichit la
compréhension du modèle, de sa performance à ses explications, puis
à ses potentiels biais.
