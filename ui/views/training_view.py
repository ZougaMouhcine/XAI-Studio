...
class TrainingView(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        self._automl_status_var = tk.StringVar(value="Pret")
    ...
    def _build(self):
        ...
        self._train_tab = self._create_scroll_tab(self._notebook, "Training Dashboard")
        self._automl_tab = self._create_scroll_tab(self._notebook, "AutoML")
        self._dl_tab = self._create_scroll_tab(self._notebook, "Deep Learning")
    ...
    def _build_training_tab(self, parent: tk.Frame):
        ...
        info.pack(fill="x")
        tk.Label(info, text="Tache", font=F.H4, bg=C.BG_CARD, fg=C.TEXT_SEC).grid(row=0, column=0, sticky="w")
        tk.Label(info, textvariable=self._task_var, font=F.H3, bg=C.BG_CARD, fg=C.TEXT).grid(row=1, column=0, sticky="w")

        tk.Label(info, text="Cible", font=F.H4, bg=C.BG_CARD, fg=C.TEXT_SEC).grid(row=0, column=1, sticky="w", padx=(24, 0))
        tk.Label(info, textvariable=self._target_var, font=F.H3, bg=C.BG_CARD, fg=C.TEXT).grid(row=1, column=1, sticky="w", padx=(24, 0))

        tk.Label(info, text="Dimensions", font=F.H4, bg=C.BG_CARD, fg=C.TEXT_SEC).grid(row=0, column=2, sticky="w", padx=(24, 0))
        ...
        tk.Label(self._models_card.inner, text="Modeles disponibles", font=F.H3, bg=C.BG_CARD, fg=C.TEXT).pack(anchor="w")

        actions = tk.Frame(self._models_card.inner, bg=C.BG_CARD)
        actions.pack(fill="x", pady=(8, 8))
        ModernButton(actions, text="Tout selectionner", style="ghost", command=self._select_all, bg=C.BG_CARD).pack(side="left", padx=(0, 8))
        ModernButton(actions, text="Tout deselectionner", style="ghost", command=self._deselect_all, bg=C.BG_CARD).pack(side="left")
        ...
        self._param_header = tk.Label(self._param_card.inner, text="Hyperparametres", font=F.H3, bg=C.BG_CARD, fg=C.TEXT)
        ...
        self._param_mode_label = tk.Label(self._param_toolbar, text="Mode auto", font=F.SMALL, bg=C.BG_CARD, fg=C.TEXT_SEC)
        ...
        tk.Label(self._train_card.inner, text="Suivi de l'entrainement", font=F.H3, bg=C.BG_CARD, fg=C.TEXT).pack(anchor="w")
    ...
    def _build_automl_tab(self, parent: tk.Frame):
        ...
        tk.Label(config.inner, text="Configuration AutoML", font=F.H3, bg=C.BG_CARD, fg=C.TEXT).pack(anchor="w")

        row = tk.Frame(config.inner, bg=C.BG_CARD)
        row.pack(fill="x", pady=(8, 8))
        tk.Label(row, text="Recherche", font=F.H4, bg=C.BG_CARD, fg=C.TEXT).pack(side="left")
        ...
        tk.Label(row, text="CV", font=F.H4, bg=C.BG_CARD, fg=C.TEXT).pack(side="left")
        ttk.Entry(row, textvariable=self._automl_cv_var, width=6).pack(side="left", padx=(8, 16))

        tk.Label(row, text="Iterations", font=F.H4, bg=C.BG_CARD, fg=C.TEXT).pack(side="left")
        ttk.Entry(row, textvariable=self._automl_iter_var, width=8).pack(side="left", padx=(8, 16))

        ModernButton(row, text="Lancer AutoML", style="primary", command=self._run_automl, bg=C.BG_CARD).pack(side="right")

        models_row = tk.Frame(config.inner, bg=C.BG_CARD)
        models_row.pack(fill="x")
        tk.Label(models_row, text="Modeles candidats", font=F.H4, bg=C.BG_CARD, fg=C.TEXT).pack(anchor="w")
        ...
        tk.Label(results.inner, text="Resultats AutoML", font=F.H3, bg=C.BG_CARD, fg=C.TEXT).pack(anchor="w")

        summary = tk.Frame(results.inner, bg=C.BG_CARD)
        summary.pack(fill="x", pady=(8, 8))
        tk.Label(summary, text="Meilleur modele", font=F.SMALL, bg=C.BG_CARD, fg=C.TEXT_SEC).grid(row=0, column=0, sticky="w")
        tk.Label(summary, textvariable=self._automl_best_var, font=F.H3, bg=C.BG_CARD, fg=C.TEXT).grid(row=1, column=0, sticky="w")

        tk.Label(summary, text="Score", font=F.SMALL, bg=C.BG_CARD, fg=C.TEXT_SEC).grid(row=0, column=1, sticky="w", padx=(24, 0))
        ...
        tk.Label(summary, text="Statut", font=F.SMALL, bg=C.BG_CARD, fg=C.TEXT_SEC).grid(row=0, column=2, sticky="w", padx=(24, 0))
        ...
        self._automl_log = LogPanel(results.inner, height=8, label="Details", bg_outer=C.BG_CARD, scrollbar=True)
    ...
    def _refresh_model_list(self):
        ...
        if not model_names:
            tk.Label(self._models_list, text="Chargez les donnees et faites le preprocessing.", bg=C.BG_CARD, fg=C.TEXT_DIM, font=F.BODY).pack(anchor="w")
            return
        ...
        for idx, name in enumerate(model_names):
            ...
            ModernButton(row, text="Configurer", style="ghost", command=lambda n=name: self._select_model(n), bg=C.BG_CARD, width=120).grid(row=0, column=1, sticky="e")
    ...
    def _select_model(self, name: str):
        ...
        self._param_header.configure(text=f"Hyperparametres - {name}")
    ...
    def _refresh_param_editor(self):
        ...
        if not name:
            tk.Label(self._param_body, text="Selectionnez un modele.", bg=C.BG_CARD, fg=C.TEXT_DIM, font=F.BODY).pack(anchor="w")
            return
        ...
        self._param_mode_label.configure(text="Mode auto")

        if use_default.get():
            tk.Label(self._param_body, text="Les parametres par defaut seront utilises.", bg=C.BG_CARD, fg=C.TEXT_DIM, font=F.BODY).pack(anchor="w")
            return

        schema = self._service.get_model_param_schema(name, include_all=self._show_all_params_var.get())
        if not schema:
            tk.Label(self._param_body, text="Aucun parametre disponible.", bg=C.BG_CARD, fg=C.TEXT_DIM, font=F.BODY).pack(anchor="w")
            return
    ...
    def _toggle_more_params(self):
        ...
        if self._show_all_params_var.get():
            self._param_toggle_btn.itemconfig(self._param_toggle_btn._text, text="Show less")
        else:
            self._param_toggle_btn.itemconfig(self._param_toggle_btn._text, text="Show more parameters")
        self._refresh_param_editor()
    ...
    def _on_train(self):
        ...
        if not selected:
            show_error("Erreur", "Selectionnez au moins un modele.")
            return
        ...
        self._train_log.set_content("Demarrage entrainement...\n")
    ...
    def _update_training_progress(self, cur: int, total: int, model_name: str, remaining: str, resources: str | None):
        ...
        self._train_status_var.set(f"Termine: {model_name} ({cur}/{total})")
        self._train_eta_var.set(f"ETA: {remaining}")
        if resources:
    ...
    def _run_automl(self):
        ...
        self._automl_status_var.set("AutoML en cours...")
        self._automl_log.set_content("Demarrage AutoML...\n")
